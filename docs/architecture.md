...
The original monolithic `bridge.py` has been split into focused modules. `bridge.py` remains as a compatibility facade that re-exports the modular implementation for existing imports.

| Object | File | Purpose |
|---|---|---|
| `VoiceLiveBridge` | `bridge_core.py` | Owns the Discord voice client, listener, audio source, Gemini bridge, watchdog, idle handling, and shutdown. |
| `GeminiLiveBridge` | `bridge_core.py` | Owns the Gemini WebSocket, setup payload, send/receive loops, reconnect, tool calls, notes, and metrics. |
| `GeminiPCMSink` | `bridge_core.py` | Receives decoded Discord PCM, filters it, downsamples it, and forwards it to Gemini. |
| `LiveAudioSource` | `bridge_audio.py` | Receives Gemini 24 kHz mono audio, converts it to Discord 48 kHz stereo frames, and implements `discord.AudioSource`. |
| Sidecar request handler and server | `bridge_http.py` | Implements the loopback HTTP routes, sidecar lifecycle, and bridge singleton. |
| Compatibility facade | `bridge.py` | Re-exports names from the focused modules so older `import bridge` call sites continue to work. |
...
10. `/voice-live-leave`, `voice_live_leave`, Discord disconnect, target-user departure, or reconnect failure stops the bridge. Sidecar `/stop` is intended to do the same but is currently unavailable because of [Issue #5](https://github.com/Capslockb/gemini-live-discord-bridge/issues/5).
...
`/notes` returns recent transcript/note events and the notes-file path. It is sensitive despite the loopback bind; do not expose the sidecar through a proxy or browser-accessible path while [Issue #4](https://github.com/Capslockb/gemini-live-discord-bridge/issues/4) remains open.

The following mutating routes are intended to require `X-API-Secret`:
...
Current `main` raises `NameError` in the mutating-route secret comparison. Treat all four routes as unavailable until [Issue #5](https://github.com/Capslockb/gemini-live-discord-bridge/issues/5) is fixed and validated on the exact head.
...
| `bridge.py` | Compatibility facade that re-exports the modular bridge implementation. |
| `bridge_config.py` | Environment configuration, constants, logging, and system prompt. |
| `bridge_context.py` | Honcho context injection. |
| `bridge_email.py` | Email address handling and reminder loop. |
| `bridge_tools.py` | Voice tool declarations and runners for web, local, GitHub, Spotify, Home Assistant, and inspection paths. |
| `bridge_opencode.py` | OpenCode tmux session management and watchers. |
| `bridge_audio.py` | PCM conversion helpers, typing SFX, and `LiveAudioSource`. |
| `bridge_core.py` | `GeminiLiveBridge`, `VoiceLiveBridge`, and `GeminiPCMSink`. |
| `bridge_http.py` | Loopback sidecar request handling, server lifecycle, and bridge singleton. |
